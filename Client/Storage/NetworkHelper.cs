...
namespace U盘文件复制
{
    ...
    public static class NetworkHelper
    {
        ...
        {
            return _clientCache.GetOrAdd(BuildClientKey(config), _ => CreateClient(config));
        }
        ...
        {
            var url = $"{config.ApiBaseUrl}/list?path={Uri.EscapeDataString(relativePath ?? "")}&recursive={(recursive ? "true" : "false")}&pageSize=10000";
            var response = await GetClient(config).GetAsync(url, ct);
            response.EnsureSuccessStatusCode();
